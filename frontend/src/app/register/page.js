...
import { Loader2, KeyRound, Mail, User, AlertCircle } from 'lucide-react';

function RegisterContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { setAuth, initializeAuth, isAuthenticated } = useAuthStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Email may be already in use or server issue.');
    } finally {
      setLoading(false);
    }
  };
