...
export default function AdminUploadOutfitPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to create outfit');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Upload New Outfit</h1>
            <p className="text-sm text-slate-400">
              Upload media to Cloudinary, configure location and category tags, and attach product buy links
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <MediaUploader value={mediaInfo} onChange={setMediaInfo} disabled={loading} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5 md:col-span-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-sm"
                />
              </div>
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Provide styling details, fabric notes, or occasion recommendations..."
                  className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-sm"
                />
              </div>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-sm"
                  />
                  <button
                    type="button"
                ...
                {locations.length > 0 && (
                  <div className="flex flex-wrap gap-2 pt-1">
                    {locations.map((loc, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-500/10 border border-amber-500/30 text-amber-300 rounded-full text-xs font-medium"
                      >
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g., Night Out, Brunch, Wedding"
                  className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-sm"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g., Minimalist, Vintage, Bohemian"
                  className="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-amber-500 text-sm"
                />
              </div>
            </div>
            ...
            {/* Multiple Product Links Section */}
            <div className="space-y-4 pt-4 border-t border-slate-800">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Add external marketplace links so users can purchase items in this outfit
                  </p>
                </div>
                <button
                  type="button"
                  onClick={handleAddProduct}
                  className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg bg-amber-500/10 text-amber-300 border border-amber-500/30 hover:bg-amber-500/20 transition-colors"
                >
                ...
                {products.map((prod, idx) => (
                  ...
                  >
                    <div className="w-full sm:w-1/3">
                      <input
                        type="text"
                        placeholder="Product Name (e.g. Linen Shirt)"
                        value={prod.productName}
                        onChange={(e) => handleProductChange(idx, 'productName', e.target.value)}
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-amber-500"
                      />
                    </div>
                    <div className="w-full sm:w-1/2">
                      <input
                        type="url"
                        placeholder="Product URL (e.g. https://myntra.com/...)"
            ...
            <div className="pt-6 border-t border-slate-800 flex items-center justify-end gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 shadow-lg hover:shadow-amber-500/20 transition-all disabled:opacity-50"
              >
                <Save className="w-4 h-4" />
                {loading ? 'Publishing Outfit...' : 'Publish Outfit'}
              </button>
            </div>
          </form>
        </div>
